...
all_moves_X = [0, 300, 600, 0, 300, 600, 0, 300, 600]
all_moves_Y = [0, 300, 600, 0, 300, 600, 0, 300, 600]
...
def drawX(x, y):
    XposX.append(x)
    XposY.append(y)
    # all_moves_X.remove(x)
    # all_moves_Y.remove(y)
...
def drawO():
    for i in range(len(XposX)):
        if pointerX == XposX[i] and pointerY == XposY[i]:
...
while running:
    ...
    for i in range(len(XposX)):
        screen.blit(XImg, (XposX[i], XposY[i]))  # Draw X
    print(board)
    pointer()
    addO(crow, ccol)
